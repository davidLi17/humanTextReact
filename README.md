...
- 默认开启「结合当前段落解释」，划词翻译直接携带所在段落；可在设置中关闭。
...
- 历史抽屉支持搜索会话标题和消息内容；选择消息命中后会定位并短暂高亮原消息，输入草稿保持在当前会话中。
...
- 在卡片填写“这次想解决什么问题？”，点击“提取对我有用的信息”，即可围绕目标提取所选原文的重点。回答下方的“原文依据”支持展开段落、查看高亮引文；这类目标提取会固定生成时的选段，重试和刷新后仍沿用原范围。
- 点击侧栏顶部“回顾对话主线”，生成当前目标、已确认结论、未解决问题和下一步，结果随会话保存。长对话会显示覆盖条数与省略提示；网页只带来源说明，图片只说明存在，本次回顾依据可读取的对话文字。
...
- 保存侧栏回答前可以编辑术语、人话释义、原句或提问和来源链接；生词本卡片会显示来源，并可安全打开网页原文。
- 点击生词本卡片的「继续问」会新建会话，把词条释义和来源填入可编辑草稿，确认后才发送给模型。
...
「结合当前段落解释」默认开启。浮窗翻译、Alt/Option+D 和右键“人话翻译”会
直接携带选区所在的有限段落；关闭后只翻译选中文字。提取段落失败时会自动按
选中文字翻译，整次操作只发起一次请求。送入侧边栏时，上下文会显示为可移除的
独立预览，并随该条消息保存，供编辑和重试继续使用。
...
- 开启时，段落会随翻译请求交给用户配置的 AI 服务；关闭时只发送选中文字。
  页面标题和 HTTP(S) 来源 URL 仅保存在本地用于预览，不发送给模型。
