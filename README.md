...
- **Zustand**: 轻量级状态管理 (1.5KB)
- **ahooks**: React Hooks 工具库
- **Tailwind CSS v4**: 原子化 CSS + Design Tokens
- **Vite**: 快速的构建工具
...
手写实现的 LRU (Least Recently Used) 缓存，用于存储翻译结果：

- **O(1) 时间复杂度**: 利用 Map 数据结构特性
- **TTL 过期机制**: 支持缓存自动过期
- **命中率统计**: 实时监控缓存效率
...
- **搜索防抖**: 300ms 延迟，减少 90% 无效过滤操作
- **useMemoizedFn**: 保持函数引用稳定，避免子组件重渲染
